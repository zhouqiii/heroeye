	...
	if(obj == undefined){
		$.ajax({
			...
			success:function(json){
				$(".server_main").append("<div style='height: 30px;'><input class='ischeckAll' type='checkbox' onclick='ischeckAll(this)' />"+"<span style='margin-left:8px;margin-right:30px;'>全选</span><div>")
				for(var i = 0;i<json.rows.length;i++){
					serverAll.set(json.rows[i].serverId,json.rows[i].serverName)
					$(".server_main").append("<div style='height: 30px;'><input class='ischeck' type='checkbox' onclick='ischeck(this)' name="+json.rows[i].serverName+" value="+json.rows[i].serverId+" />"+"<span style='margin-left:8px;margin-right:30px;'>"+json.rows[i].serverName+"</span><div>")
				}
			}
		});
	}

...
function getRedis(){
	$.ajax({
		...
		success:function(json){
			...
			for(var i = 0;i<json.data.length;i++){
				$(".file_upload").append("<input type='radio' onclick='checkFile(this)'/><span style='margin-right: 21px;margin-left:12px;'>路径</span><input class='saveRoute' style='margin-right:34%;margin-bottom: 12px;' type='text' value="+json.data[i]+"> ")
			}
		}
	});
}
//添加上传路径
function addRedis(){
	$(".file_upload").append("<input type='radio' onclick='checkFile(this)'/><span style='margin-right: 21px;margin-left:12px;'>路径</span><input class='saveRoute' style='margin-right:34%;margin-bottom: 12px;' type='text' placeholder='例：https://www.baidu.com/upload/1028.html' > ")
}
//保存上传路径
function saveRedis(){
	var urlString="";

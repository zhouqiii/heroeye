...
function postsend(){
	...
	var one_currencyID = sendRep.oneMoneyType;//一号货币ID
	var one_currencyNum = $("#one_currencyNum").val();//一号货币数量
	var two_currencyID = sendRep.twoMoneyType//二号货币ID
	...
	var supplement_number = $("#supplement_number").val();//补单单号
	var supplement_amount = $("#supplement_amount").val();//补单充值金额
	var regEn = /[`~!@#$%^&*()_+<>?:"{},.\/;'[\]]/im;
	var regCn = /[·！#￥（——）：；“”‘、，|《。》？、【】[\]]/im;
	var reg=/(^\s+)|(\s+$)/g;
	//判断是否开启补单
...
//		supplement_amount = '';
//	}
	if(supplement_number == ""||supplement_number == null||input.test(supplement_number)){
		alert("补单单号不能为空");
		return false;
	}else if(regEn.test(supplement_number) || regCn.test(supplement_number) || reg.test(supplement_number)){
		alert("补单单号不能为特殊字符");
		return false;
	}else if(supplement_amount == ""||supplement_amount == null||input.test(supplement_amount)){
		alert("补单充值金额不能为空");
		return false;
	}else if(regEn.test(supplement_amount) || regCn.test(supplement_amount) || reg.test(supplement_amount)){
		alert("补单充值金额不能为特殊字符");
		return false;
	}
	else if(roleID == ""||roleID == null||input.test(roleID) ){
		alert("角色ID不能为空");
		return false;
		...
		layer.confirm('是否发送补单', function(index) {
			$("body").mLoading("show")
			$.ajax({
				type:"get",

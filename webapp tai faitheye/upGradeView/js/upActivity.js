layui.use(['element', 'table', 'layer', 'upload'], function () {
    ...
    var duration='持续天数'
    var cycleday='循环天数'
    var paramArr5='生效服务器类型'
    var annouce='发布状态'
    var opration='操作'
    	var activity 
    //第一个实例
   var innitData
   function getInnitData(){
    	$.ajax({
    		type:"get",
        ...
        cols: [
            [ //表头
                ...
                templet: function (d) {
                    ...
                    if (d.state == 0) {
                        return type = "<span style='color:#ffffff;' class='noAnnunoce' >不可发布</span>"

                    }
                    if (d.state == 1) {
                        return type = "<span style='color:#ffffff;' class='annunoce' >发布异常</span>"

                    }
                    if (d.state == 2) {
                        return type = "<span style='color:#ffffff;' class='annunoce' >已发布</span>"

                    }
                    if (d.state == 3) {
                        return type = "<span style='color:#ffffff;' class='annunoce' >未发布</span>"
...
//            },
            {
                ...
                templet: function (value) {
                    return `
                        ...
                        <span class='start_up' lay-event='del' style="color:#e64242;">删除</span> 
...
//    	$('#but-removeBatch').css("display","block")  //未发布状态下显示发布按钮
    ...
    table.on('tool(test)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        //编辑详情
            ...
            self.location.href = "upActivityEdit.html"
        }
        //查询服务器
...
//        				  if(e.data[i].paramArr5 == array[i].children[9].textContent){
//        					  if(e.data[i].attributeId == array[i].cells[2].innerText){
...
function checboxs(obj) {
    ...
    if (obj == undefined) {
        ...
        $(".right_content").empty()
        $(".right_content").append("<input type='checkbox' value=''  class='checkRightAll' onclick='checkRightAll(this)' id='rightAll'/><span style='width:80%;'>全选</span>")
        $.ajax({
            type: "get",
            url: "../getServerListNoPage.action",
            async: false,
            ...
            success: function (json) {
                for (var i = 0; i < json.rows.length; i++) {
                    $(".right_content").append("<input type='checkbox' onclick='cancelRightAll(this)' value=" + json.rows[i].serverId + " />" + "<span class='content'>" + json.rows[i].serverName + "</span>")
                }
            }
        });
    }
...
//        $(obj).siblings().removeAttr("checked")
        $(".right_content").empty()
        $(".right_content").append("<input type='checkbox' value='' onclick='checkRightAll(this)' id='rightAll'/><span style='width:80%;'>全选</span>")
//        areaid = $(obj).val();
...
function cancelRightAll(){
	var checkNum=0
	for(var i=1;i<$(".right_content input").length;i++){
		if($(".right_content input")[i].checked == true){
			checkNum+=1
		}
	}
	if(($(".right_content input").length-1)===checkNum){
...
//        $(".right_content").append("<input type='checkbox' /><span style='width:80%;'>全选</span>")
        $.ajax({
            ...
            success: function (json) {
                for (var i = 0; i < json.rows.length; i++) {
                    $(".right_content").append("<input type='checkbox' onclick='cancelRightAll(this)' checked='checked' value=" + json.rows[i].serverId + " />" + "<span class='content'>" + json.rows[i].serverName + "</span>")
                }
            }
        });
...
function checkLeftAll(obj){
	...
	if(obj.checked == true){
		...
		$(".right_content").empty()    
		$(".right_content").append("<input type='checkbox' value='' onclick='checkRightAll(this)' checked id='rightAll'/><span style='width:80%;'>全选</span>")
//	      for(var i=0;i<serverChecked.length;i++){
//	      	areaid = serverChecked[i]
	      	$.ajax({
	              type: "get",

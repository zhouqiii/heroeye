...
function postsend() {
	...
	var jsons = arr.join(','); //服务器id
//	var notice = $("#seltype").val();//公告类型
//	var interval = $("#seltime").val();//时间
//	var index=interval.indexOf('~')
//	var startTime=interval.substring(0,index)//开始时间
//	var endTime=interval.substring(index+1)//结束时间
...
//		return false
//	}
//	var add_dele = $("#selheight").val();//滚动速度
//	var scrolltimes = $("#seljian").val();//滚动间隔
//	if(scrolltimes===''||scrolltimes===null){
//		alert('请输入滚动间隔')
//		return false
//	}
//	if(parseInt(scrolltimes)===0){
//		alert('滚动间隔不能为0')
//		return false
//	}
//	var activeNum = $("#selnum").val();//活动次数
//	if(activeNum===''||activeNum===null){
//		alert('请输入活动次数')
//	}
//	if(parseInt(activeNum)===0){
//		alert('活动次数不能为0')
//		return false
//	}
//	var anoContent = $("#seltextarea").val();//公告内容
//	if(anoContent===''||anoContent===null){
//		alert('请输入公告内容')
//		return false
//	}
	var regEn = /[`~!@#$%^&*()_+<>?:"{},.\/;'[\]]/im;
	...
	if(notice===''||notice===null){
	...
	else if(regEn.test(notice) || regCn.test(notice) || reg.test(notice) ) {
        alert("公告ID不包含特殊字符");
        return false;
    }
	...
	else if(regEn.test(interval) || regCn.test(interval) || reg.test(interval) ) {
        alert("间隔时间不包含特殊字符");
        return false;
    }else{
    	$("body").mLoading("show")
    	$.ajax({
    		...
    		success:function(json){
    			...
    			var dom=document.getElementById('contentBack')

	...
	$.ajax({
		...
		success: function(json) {
			for(var i = 0; i < json.data.length; i++) {
				...
				str = str.replace(/ /g, '&#32;')
				$(".check_inner").prepend("<p class='check_sp1'><span class='filename'><input value="+str+" type='checkbox'/>" + json.data[i].fileName  + "</span><span class='filetime'>" + json.data[i].lastDate + "</span><span class='filesize'>"  + json.data[i].fileSize + "</span><el-tooltip class='item' effect='dark' content='下载' placement='right'><span style='cursor:pointer' onclick='download(this)' id=" + str + "><img src='../img/导出.png'></span></el-tooltip></p>")
			}
		}
	});
}
